...
contract carRental {
    ...
    event AddNewCar(uint256 carId, string model, address owner);
    event RentalRequest(uint256 carId, uint256 dayToRent, uint256 startDate);
    event DeliveryCar(uint256 carId, uint256 rentId, uint256 price);
    event ReturnCar(uint256 carId, uint256 rentId, uint256 dueDate);
    ...
    function addNewCar(string memory _model, uint256 _dailyWages) external {
        ...
        emit AddNewCar(currentId, _model, msg.sender);
    }
        ...
        payable
    {
        ...
        emit RentalRequest(_carId, _dayToRent, block.timestamp);
    }
    ...
    function deliveryCar(uint256 _carId, uint256 _rentId) external payable {
        ...
        emit DeliveryCar(_carId, _rentId, calculatedPrice);
    }
    ...
    function returnCar(uint256 _carId, uint256 _rentId) external payable {
        ...
        emit ReturnCar(_carId, _rentId, dueDate);
    }
